            ...
            <article class="flex flex-col gap-2 mx-auto my-3 w-11/12 md:w-10/12 lg:w-3/5 text-xl">
                @foreach ($keys->take(1) as $key)
                    @foreach ($footers->take(1) as $footer)
                        ...
                        <p>Email : {{ $footer->email }} (Silahkan gunakan PGP untuk komunikasi e-mail terenkripsi) PGP Key
                            dapat diunduh <a class="underline decoration-primary" href="{{ 'storage/' . $key->path }}">di
                                sini</a></p>
