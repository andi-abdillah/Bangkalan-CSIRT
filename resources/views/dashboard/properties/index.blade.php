    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="p-3">
            <img src="{{ asset('storage/' . $property->image) }}" alt="{{ $property->name }}" class="w-32 rounded-lg">
          </td>
          <td class="p-3 flex gap-2">
            <a class="icon-edit tooltip" data-tip="Edit" href="/dashboard/properties/{{ ($property->slug) }}/edit">
              <span class="material-symbols-rounded">
                edit
              </span>
            </a>
            <form class="tooltip" data-tip="Delete" action="/dashboard/properties/{{ ($property->slug) }}" method="post">
              @method('delete')
              @csrf
